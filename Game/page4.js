...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function randomizeInitialColors() {
        ...
        mainDivs.forEach(function (div, index) {
            if (index === 0 || index === 2 || index === 4 || index === 7 || index === 12 ||index === 15 || index === 16 || index === 17 || index === 18 || index === 19 || index === 22 || index === 27 || index === 30 || index === 32 || index === 34) {
                ...
                // Otherwise, set a shuffled color
    ...
    // randomize initial colors everytime the page is relaoded 
    ...
    for (let row = 0; row < 7; row++) {
        for (let col = 0; col < 5; col++) {
            ...
            cell.id = 'cell-' + cellCount; // Assign diff. ID to each cell
...
gridContainer.addEventListener('click', (event) => {
    ...
    // if the target div is one of the specified fixed divs
    if (targetDiv.classList.contains('grid-cell') && 
        ...
        return; // Do nothing for the specified cells
    }
    ...
    let audioPlayed = false; // to check if audio has been played

    //  predetermined colors for each box
    ...
    function cycleColors() {
        if (selectedDiv && spaceBarPressed) {
            ...
            if (selectedDiv.style.backgroundColor === predeterminedColors[selectedDiv]) {
                // Play audio 
                if (!audioPlayed) {
                    audio.play();
                    audioPlayed = true;
    ...
    function handleDivSelection(div) {
        if (selectedDiv !== div) {
            if (selectedDiv) {
                // Remove the previous border class
                selectedDiv.classList.remove('selected-border');
            }
            selectedDiv = div;
            // Add the border class to the selected div
